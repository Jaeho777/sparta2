...
### 6.2 Baseline 모델 결과 (Validation 기간)

| 모델 | VAL RMSE | VAL MAPE | VAL MAE |
|------|----------|----------|---------|
| **GradientBoosting** | **395.83** | 2.27 | 341.89 |
| CatBoost | 433.01 | 2.23 | 335.34 |
| AdaBoost | 511.37 | 3.11 | 468.99 |
| LightGBM | 763.83 | 4.23 | 635.11 |
| XGBoost | 766.31 | 4.36 | 654.74 |

**발견**: 예상과 달리 가장 기본적인 GradientBoosting이 1위를 차지했다. 이는 데이터의 패턴이 비교적 단순하여 복잡한 모델이 과적합되었을 가능성을 시사한다.

**교훈**: "복잡한 모델이 항상 좋은 것은 아니다"

### 6.3 Residual 스태킹 결과 (Validation 기간)

GradientBoosting을 베이스라인으로 하여 Residual 스태킹을 수행했다.

| Base | Residual | VAL RMSE |
|------|----------|----------|
| AdaBoost | AdaBoost | 501.03 |
| CatBoost | AdaBoost | 512.75 |
| GradientBoosting | AdaBoost | 529.02 |
| GradientBoosting | GradientBoosting | 552.70 |
...
### 6.4 ROR 스태킹 결과 (Validation 기간)

상위 Residual 조합을 기반으로 ROR 스태킹을 수행했다.

| Base | Residual | ROR | VAL RMSE |
|------|----------|-----|----------|
| GradientBoosting | GradientBoosting | CatBoost | 800.22 |
| GradientBoosting | GradientBoosting | GradientBoosting | 807.84 |
| GradientBoosting | AdaBoost | GradientBoosting | 833.19 |
...
| 모델 | RMSE | MAPE | MAE |
|------|------|------|-----|
| **Naive_Drift** | **480.67** | **2.10** | **325.76** |
| Naive_Last | 569.23 | 2.58 | 410.00 |
| ROR_GB+Cat+LGBM | 1264.73 | 6.01 | 949.91 |
| BASE_GradientBoosting | 1312.09 | 5.73 | 937.62 |
| BASE_CatBoost | 1332.73 | 6.22 | 1005.85 |
...
| 가중치 조합 | RMSE | 기존 대비 |
|-------------|------|-----------|
| Naive*0.5 + GB*0.5 | 761.59 | +58% (악화) |
| Naive*0.7 + GB*0.3 | 505.65 | +5% (약간 악화) |
| Naive*0.8 + GB*0.2 | 431.15 | -10.3% |
| **Naive*0.9 + GB*0.1** | **421.51** | **-12.3%** |

**발견**: **Naive에 소량의 GradientBoosting(10%)을 결합**하면 순수 Naive보다 더 좋은 성능을 보인다. 하지만 GradientBoosting 비중이 높아질수록 성능이 급격히 악화된다.
...
| 순위 | 모델 | RMSE | 비고 |
|------|------|------|------|
| 1 | **Hybrid(Naive*0.9 + GB*0.1)** | **421.51** | **최고 성능 (-12.3%)** |
| 2 | Hybrid(Naive*0.8 + GB*0.2) | 431.15 | -10.3% |
| 3 | Naive_Drift_Damped(α=0.7) | 438.60 | 단순 변형 중 최고 |
| 4 | Naive + GB(Residual) | 461.55 | 스태킹 성공 |
| 5 | Naive_Drift (기존) | 480.67 | 기준 |
| 6 | Naive + Residual + ROR | 497.14 | 스태킹 실패 (과적합) |

**핵심 인사이트**:
1. **ROR(3단계)은 실패**: 복잡한 구조보다는 단순한 보정이 낫다
2. **Hybrid가 스태킹보다 강함**: Naive의 추세 추종력과 GradientBoosting의 패턴 인식을 가중 평균하는 것이 가장 효과적
3. **Naive가 메인, GradientBoosting은 보조**: GB 비중 10%일 때가 최적
...
| 우선순위 | 모델 | RMSE | 특징 |
|----------|------|------|------|
| 1순위 | Hybrid(Naive*0.9 + GB*0.1) | 421.51 | 최고 성능 |
