...
| 시기 | 이벤트 | 가격 영향 | 원인 |
|------|--------|-----------|------|
| 2015-16 | Supply Glut | $20,000 → $8,000 (-60%) | 인도네시아 광석 수출 금지 해제, 중국 수요 둔화 |
| 2020.03 | COVID-19 | $13,000 → $11,000 (-15%) | 글로벌 제조업 셧다운 |
| 2022.03 | Russia-Ukraine | $25,000 → $35,000 (+40%) | 러시아(세계 3위 생산국) 제재 우려, LME 숏스퀴즈 |
| 2023-25 | 정상화 | $15,000 안정 | 인도네시아 증산, 수급 균형 |
...
| 평균 | ≈0% | 장기적으로 방향성 없음 |
| 표준편차 | ~4% | 주간 변동폭 |
| 첨도 | >3 | Fat Tail (극단값 빈발) |
| 왜도 | <0 | 급락이 급등보다 빈번 |

**시사점**:
- 극단값에 민감한 모델(선형회귀)보다 **트리 기반 모델이 더 적합**
- Fat Tail로 인해 리스크 지표 산출 시 정규분포 가정은 위험 과소평가
- 다만, Test 기간 트리 모델이 Naive보다 저조한 이유는 **시장 구조 변화**(일방적 급등 추세) 때문
...
|------|---------------|------|
| 평상시 | 20~30% | 정상 시장 |
| 2015-16 | 35~40% | 중국 경기 불안 |
| 2020.03 | 45% | COVID-19 쇼크 |
| 2022.03 | **60%+** | 러시아-우크라이나 전쟁 |
| 2025 Test기간 | 25% | 안정적 (Naive 유리) |
 
**시사점**:
- 변동성은 시간에 따라 크게 변한다 **(이질분산성)**
- Test 기간(2025.10~2026.01)은 **저변동성 구간**으로, Naive 모델에 유리한 환경
- 고변동성 구간에서는 ML 모델이 상대적으로 더 나은 성능 가능

### 4.4 STL 분해 분석 (Seasonal-Trend Decomposition using LOESS)

시계열의 구조적 특성을 파악하기 위해 STL 분해를 수행했다. STL은 시계열을 Trend(추세), Seasonal(계절성), Residual(잔차)의 세 구성요소로 분리하는 업계 표준 기법이다.

![STL Decomposition](report_images/stl_decomposition.png)

**STL 분해 결과**:

| 구성요소 | 측정값 | 해석 |
|----------|--------|------|
| **Trend Strength** | 0.79 | 강한 추세 존재 (0.6 이상이면 강함) |
| **Seasonal Strength** | ≈0 | 계절성 미미 (52주 주기 패턴 없음) |
| **Trend Range** | $9,510 ~ $21,904 | 약 130% 가격 변동폭 |
| **Residual Std** | 2,165 | 설명되지 않는 노이즈 수준 |

**핵심 인사이트**:
1. **강한 추세(0.79)**: Naive_Drift 모델이 효과적인 이론적 근거. 추세가 강할수록 "내일도 오늘과 같은 방향"이라는 가정이 유효
2. **미미한 계절성(≈0)**: 연간 52주 주기의 반복 패턴이 없음. 계절 조정(seasonal adjustment) 불필요
3. **실무 적용**: 계절성 분해 모델(SARIMA 등)보다 추세 중심 모델(Naive_Drift, Holt's Linear)이 적합

### 4.5 ADI-CV 프레임워크 (수요 예측 분류)

공급망 및 재고 관리 분야에서 널리 사용되는 **ADI-CV 프레임워크**를 적용하여 니켈 가격 시계열의 예측 난이도를 분류했다.

![ADI-CV Classification](report_images/adi_cv_classification.png)

**ADI-CV 분류 기준**:
- **ADI (Average Demand Interval)**: 유의미한 가격 변동(±2% 이상) 사이의 평균 간격
- **CV (Coefficient of Variation)**: 수익률의 변동 계수 (표준편차/평균)

**분류 결과**:

| 지표 | 측정값 | 임계값 | 판정 |
|------|--------|--------|------|
| ADI | 1.99주 | 1.32 | > 임계값 (간헐적) |
| CV | 0.85 | 0.49 | > 임계값 (변동성 큼) |
| **분류** | **Lumpy** | - | 예측 난이도 최상 |
...
| 분류 | ADI | CV | 특성 | 권장 모델 |
|------|-----|-----|------|-----------|
| Smooth | < 1.32 | < 0.49 | 일정하고 예측 가능 | 단순 이동평균, 지수평활 |
| Erratic | < 1.32 | ≥ 0.49 | 빈번하지만 변동성 큼 | ML 모델, Robust 방법 |
| Intermittent | ≥ 1.32 | < 0.49 | 간헐적이지만 안정적 | Croston 방법 |
| **Lumpy** | ≥ 1.32 | ≥ 0.49 | **간헐적 + 변동성 큼** | **예측 어려움, 앙상블 필요** |

**핵심 인사이트**:
1. 니켈 가격은 **Lumpy** 분류로, 예측이 가장 어려운 유형에 해당
2. **높은 ADI(1.99)**: 2주에 한 번 정도만 유의미한 가격 변동 발생 → 노이즈 대비 신호가 약함
3. **높은 CV(0.85)**: 변동이 발생할 때 크기가 불규칙 → 단순 모델로 포착 어려움
4. **모델 선택 근거**: Lumpy 데이터에서 ML 모델의 저조한 성능은 예상된 결과. Naive 모델이 오히려 robust한 이유 설명

### 4.6 ADF 정상성 검정 (Augmented Dickey-Fuller Test)

시계열의 정상성(stationarity)을 검증하여 적합한 모델링 접근법을 결정했다.

| 데이터 | ADF 통계량 | p-value | 정상성 |
|--------|------------|---------|--------|
| 원본 가격 | -1.74 | 0.409 | **비정상 (Non-stationary)** |
| 1차 차분 (수익률) | -20.68 | 0.000 | **정상 (Stationary)** |

**해석 및 실무 적용**:
1. **원본 가격 비정상**: 가격 레벨 그대로 ML 모델에 입력하면 spurious regression 위험
2. **수익률 정상**: 차분 후 정상성 확보 → ML 모델의 수렴 안정성 보장
3. **이중 접근법 정당화**:
...
| 그룹 | 주요 피처 | 경제적 의미 |
|------|-----------|-------------|
| **수급 지표** | Ni/Al/Zn 재고 | 재고 증가 → 공급 과잉 → 가격 하락 압력 |
| **거시경제** | US/AUS/BRZ 금리 | 금리 상승 → 달러 강세 → 원자재 가격 하락 |
| **연관 금속** | Sn, Steel | 비철금속 동조화 현상 반영 |

**핵심 발견**:
- 니켈 가격에 가장 큰 영향을 미치는 것은 **수급 지표(재고)**와 **거시경제 지표(금리)**
- 예상과 달리 **중국 직접 지표(CSI300)보다 호주/브라질 금리**가 더 중요 → 원자재 수출국 경기가 핵심
- **가설 1 (비철금속 동조화) 확인**: Sn, Al, Zn 재고/가격이 상위권
