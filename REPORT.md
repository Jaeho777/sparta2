...
sparta2의 Hybrid(RMSE 406.80)를 개선하기 위해 `sparta2_advanced.ipynb`에서 체계적인 추가 실험을 수행했다.

#### 1.5.1 실험 구조 (Phase 기반)

| Phase | 내용 | 목적 |
|-------|------|------|
| 0 | 환경 설정 | 재현성 확보 |
| 1 | 기준선 재현 | RMSE 406.80 확인 |
| 2 | 피처 엔지니어링 | RV, Momentum, Z-score |
| 3 | Regime Detection | 적응형 가중치 |
| 4 | 앙상블 개선 | 다중 ML + 가중치 탐색 |
| 5 | Time Series CV | 교차 검증 |
| 6 | 최종 비교표 | 자동 결론 생성 |

#### 1.5.2 피처 엔지니어링 결과 (Phase 2)

| 피처 | 설명 | 결과 |
|------|------|------|
| Realized Volatility | 4/8/12/26주 윈도우 변동성 | 개선 없음 |
| Momentum (4w, 12w) | 가격 변화율 | 개선 없음 |
| Z-score (12w, 26w) | 평균 대비 편차 | 개선 없음 |

> **분석**: shift(1) 적용으로 Data Leakage 방지 후 테스트, 동일 테스트셋으로 공정 비교

#### 1.5.3 Regime Detection 결과 (Phase 3)

- **방법**: 12주 변동성 중위값 기준 국면 분류 (Low/High Volatility)
- **적응형 가중치**: Low Vol → Naive 70%, High Vol → Naive 90%
- **결과**: 기대보다 개선 미미 (소규모 Test 샘플의 한계)

#### 1.5.4 앙상블 개선 결과 (Phase 4)

다양한 Naive/ML 가중치 조합 테스트:

| 조합 | Test RMSE | 기준선 대비 |
|------|-----------|------------|
| Naive*0.9+LGB*0.1 | ~420 | ❌ 악화 |
| Naive*0.8+XGB*0.2 | ~415 | ❌ 악화 |
| Naive*0.8+GB*0.2 | **406.80** | ✅ 기준선 |
| Naive*0.7+LGB*0.3 | ~430 | ❌ 악화 |

> **결론**: Naive 80% + GB 20% 조합이 최적

#### 1.5.5 Time Series CV 결과 (Phase 5)

5-Fold 시계열 교차 검증 수행:
- CV 평균 RMSE와 표준편차로 모델 안정성 평가
- 결과: 단일 GradientBoosting의 성능이 폴드마다 불안정 → Hybrid가 더 안정적

#### 1.5.6 고급 실험 핵심 결론

> **⚠️ 핵심 발견**: Test 기간(2025.10~2026.01)이 일방적 상승 추세여서 Naive 모델이 자연스럽게 우위를 차지했다.
>
> **시장 레짐 변화 (Regime Change)** 분석:
> - **Train 기간**: 평균회귀 패턴 (가격이 평균으로 회귀)
> - **Test 기간**: 추세 지속 패턴 (일방적 상승)
> - **결과**: ML 모델들이 과거 패턴에 과적합, 새로운 레짐에 적응 실패

#### 1.5.7 향후 연구 방향

1. **Market Regime Detector 개발**: 자동 국면 감지 시스템
2. **동적 가중치 조절**: 시장 상태에 따른 Naive/ML 비중 변경
3. **더 긴 Test 기간**: 다양한 시장 상황 포함한 재검증
