...
|------|------|-------------|------|
| **Hybrid_Naive0.8_LGB0.2** | **398.00** | **-8.80 (개선)** | Validation 기반 GridSearch |
| Hybrid_Naive0.8_GB0.2 (기준선) | 406.80 | 기준 | sparta2 |

> **주의**: 수치는 노트북 재실행 후 shift(1) 수정이 반영되면 변경될 수 있다. 본 보고서의 RMSE는 수정 전 코드 기준이며, 수정 후 재실행 결과를 최종으로 갱신해야 한다.
...
| 지표 | 값 |
|------|-----|
| Validation RMSE (Hybrid 0.8:0.2) | 225.96 |
| Test RMSE (Hybrid 0.8:0.2) | 398.00 |
| 차이 비율 | 76% |

Validation과 Test의 RMSE 차이가 76%로 매우 크다. 이는 Validation 기간(횡보, 변동성 0.07)과 Test 기간(급등, 변동성 0.26)의 시장 레짐이 완전히 다르기 때문이다. 이 차이는 모델 과적합이 아닌 **시장 구조 변화**에 기인한다.
...
→ LGB가 GB보다 매 주차 평균 약 $44/톤 (= 8.80 / 0.2) 더 정확한 보정값을 제공
```

LightGBM이 GradientBoosting 대비 평균 $44/톤 더 정확한 예측을 제공하여, 20% 가중치에서도 전체 RMSE를 유의미하게 개선시켰다.
...
|------|------|------|-----------|-------------|------|
| **1** | **GridSearch (LGB)** | **Hybrid Naive x 0.8 + LGB x 0.2** | **398.00** | **+8.80 개선** | Validation 기반 |
| 2 | Damped Naive | Damped + LGB | 가변 | - | Validation에서 최적 phi 선택 |
| 3 | Baseline 재현 | Hybrid (Naive x 0.8 + GB x 0.2) | 422.20 | -15.40 | 주1) |
| 4 | Stacking | Naive + 20% Residual | 465.89 | -59.09 | |
| 5 | LSTM | LSTM (lookback=4) | 1,105.43 | -698.63 | |
| 6 | ARIMA | ARIMA(3,1,2) | 1,211.88 | -805.08 | |

> **주1)** 기준선 재현 시 RMSE 422.20으로 sparta2의 406.80과 15.40 차이가 발생했다. 원인: sparta2에서는 SHAP으로 선별된 19개 피처를 사용했으나, sparta2_advanced에서는 73개 전체 피처를 사용했기 때문이다. 피처 수가 많을수록 GB의 과적합 위험이 증가한다.

### 6.2 0130 vs 0207 주요 모델 비교

| 모델 | Source | RMSE | 비고 |
|------|--------|------|------|
| **Hybrid_Naive0.8_LGB0.2** | **0207** | **398.00** | **최고 성능** |
...
Stacking은 Naive 단독(480.67)보다는 소폭 개선되지만 Hybrid(398.00)에 미치지 못한다. ML의 잔차 보정이 Train 기간의 평균회귀 패턴에 기반하므로, Test 기간의 급등 추세에서는 역방향 보정이 발생한다.
...
sparta2(RMSE 406.80) → sparta2_advanced(RMSE 398.00)의 개선:

| 요인 | 기여 | 설명 |
|------|------|------|
| **ML 모델 교체** (GB → LGB) | 핵심 | 동일 0.8:0.2에서 406.80 → 398.00 |
...
수정 전 RMSE 398.00은 이 누수의 영향을 받았을 수 있으므로, **노트북을 재실행하여 shift(1) 적용 후의 최종 RMSE를 확인해야 한다**.
...
1. **RMSE 406.80 → 398.00으로 개선** (약 2.2% 감소)
...
sparta2_adv (0207): Naive_Drift x 0.8 + LightGBM(n=50,d=5) x 0.2     → RMSE 398.00 (개선 -8.80)
```
...
> **노트북 재실행 필요**: 본 보고서에 기술된 코드 수정(shift(1) 적용, Validation 기반 가중치 최적화 등)을 반영하려면 노트북을 재실행해야 한다. 최종 RMSE는 재실행 후 확정된다.
