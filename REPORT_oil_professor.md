...
## 결론 요약
- 본 보고서는 세 개의 제출 노트북이 각각 무엇을 검증하기 위해 작성되었는지와, 그 노트북별 결과가 같은 조건의 반복 검증에서 어떻게 다시 확인되는지를 함께 정리한 문서이다.
- 기준 성능 재현 노트북에서는 메인 브랜치의 니켈 예측 구조를 유가에 그대로 적용하였다. 이때 가장 좋은 모형은 `Hybrid_Naive0.7_GB0.3`이었고, 시험 RMSE는 `1.3440`이었다.
- Transformer 심화 실험 노트북에서는 SHAP 기반 변수 선택, 시계열 교차검증, transformer 계열 조합 비교, 잔차 보정, 2차 잔차 보정을 단계적으로 수행하였다. 단일 분할 기준으로 검증 RMSE가 가장 낮았던 공식 선정 모형은 `PatchTST + Transformer + ElasticNet` 구조였고, 최종 검증 RMSE는 `1.2443`이었다.
- 같은 노트북에서 시험 구간 기준 가장 낮은 오차를 보인 조합은 `PatchTST + iTransformer`였고, 시험 RMSE는 `1.2308`이었다. 다만 이 조합은 검증 기준 상위 3개에는 포함되지 않았으므로 공식 선정 모형으로 채택하지 않았다.
- STL 구조분해 보조 노트북은 `STL -> Exponential Smoothing -> NLinear -> LightGBM` 순서의 구조 분해 접근을 점검하는 보조 실험이다. 단일 분할 기준으로는 `Exponential Smoothing + NLinear + LightGBM` 구조가 시험 RMSE `1.2275`를 기록하였다.
- 그러나 위 세 결과는 각 노트북의 목적에 맞춘 단일 분할 실험 결과이므로, 이 숫자만으로 서로 다른 계열의 모형 우열을 단정해서는 안 된다.
- 직접 비교를 위해 추가한 공통 반복 검증에서는 모든 후보를 동일한 `12주 검증 + 12주 시험` rolling train/validation/test 프로토콜 아래 다시 평가하였다. 그 결과 평균 시험 RMSE 기준으로는 `RandomWalk_1step`이 `2.2191`, `ExpSmoothing`이 `2.3195`, `ExpSmoothing+NLinear`이 `2.4743`, `ExpSmoothing+NLinear+LightGBM`이 `2.5089`였고, transformer 조합들은 이보다 높았다.
- 따라서 단일 분할에서는 일부 복잡한 조합이 매우 낮은 시험 오차를 기록했지만, 동일 조건 반복 검증에서는 그 우위가 안정적으로 재현되지 않았다. 특히 2차 잔차 보정은 검증 구간에서는 여러 번 도움을 주었지만, 반복 시험 구간에서 일관된 추가 개선을 만들었다고 쓰기는 어렵다.

**표 1. 제출 노트북의 단일 분할 결과 요약**
...
| STL 구조분해 실험 최종 구조 | STL 구조분해 보조 노트북 | Exponential Smoothing | NLinear | LightGBM | 2.7126 | 1.2275 | STL 보조 실험 내부에서 가장 낮은 시험 오차를 기록한 구조 |
...
직접 비교를 위해 본 보고서에서는 추가 확인용 스크립트 `oil_common_protocol_backtest.py`도 사용하였다. 이 스크립트는 제출 노트북을 대체하는 파일이 아니라, 서로 다른 노트북에서 나온 결과를 같은 rolling train/validation/test 조건 아래 다시 비교하기 위한 보고서용 검증 도구이다.
...
### 3.6 직접 비교를 위한 공통 반복 검증 프로토콜
서로 다른 노트북의 결과를 직접 비교할 때 가장 중요한 원칙은 모든 후보를 같은 정보 집합과 같은 분할 규칙 아래 다시 평가하는 것이다. 이를 위해 본 보고서에서는 `oil_common_protocol_backtest.py`를 사용하여 추가 반복 검증을 수행하였다.
...
- 모든 후보는 각 시점 `t`를 예측할 때 시점 `t-1`까지 관측된 실제 정보만 사용하였다. 즉 공통 반복 검증에서의 benchmark는 `RandomWalk_1step`이다.
- 기준 성능 재현 노트북의 혼합 가중치는 각 반복 원점의 검증 구간에서 다시 선택하였다.
- Transformer의 2차 잔차 보정과 STL의 2차 잔차 보정은 각 반복 원점의 검증 구간에서 성능이 개선될 때만 시험 구간에 반영하였다.

따라서 본 보고서에서 서로 다른 계열의 모형을 직접 비교할 때는 단일 분할 결과보다 이 공통 반복 검증 결과를 우선적으로 해석한다.
...
종합하면, Transformer 심화 실험 노트북의 단일 분할 결과에서는 2차 잔차 보정이 일부 상위 조합에서 추가 개선을 보였다. 다만 이 결론은 하나의 검증 구간과 하나의 시험 구간에 근거한 것이므로, 일반적 우위로 해석하지 않고 뒤의 공통 반복 검증 결과와 함께 읽어야 한다.
...
따라서 STL 구조분해 보조 노트북은 구조 분해 접근 안에서 `잔차 보정`과 `2차 잔차 보정`이 어떤 역할을 하는지 보여주는 보조 실험으로 해석하는 것이 맞다. 이 노트북의 단일 분할 결과는 유의미하지만, 다른 계열의 모형과 직접적인 우열 비교는 뒤의 공통 반복 검증에서 다시 확인해야 한다.

## 7. 동일 조건 반복 검증 결과
### 7.1 공통 반복 검증 평균 성능
직접 비교를 위해 다섯 개의 반복 원점에서 같은 train/validation/test 규칙을 적용한 결과는 다음과 같다.

**표 12. 공통 반복 검증 평균 성능 비교**
| 계열 | 모형 | 평균 검증 RMSE | 검증 RMSE 표준편차 | 평균 시험 RMSE | 시험 RMSE 표준편차 | 반복 수 |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| Benchmark | RandomWalk_1step | 2.5866 | 0.8101 | 2.2191 | 0.9941 | 5 |
...
| Baseline | Hybrid_NaiveDrift_GB | 3.2192 | 1.0236 | 2.7912 | 1.2916 | 5 |
...
### 7.3 직접 비교 해석
- 반복 원점 전체를 평균하면 가장 안정적인 benchmark는 `RandomWalk_1step`이었다.
...
3. 단일 분할 기준으로는 Transformer 심화 실험과 STL 구조분해 실험 모두에서 2차 잔차 보정이 유리해 보이는 사례가 있었다. 그러나 동일 조건 반복 검증에서는 이 추가 이득이 평균 시험 성능으로 안정적으로 이어지지 않았다.
...
5. STL 구조분해 보조 노트북은 구조 분해 관점의 보조 실험으로서 의미가 있다. 다만 다른 계열의 모형과 직접 비교하면, 평균 성능 기준에서는 `RandomWalk_1step`과 `ExpSmoothing` 계열이 더 안정적이었다.
...
| STL 구조분해 보조 노트북: 2단계 구조 | Exponential Smoothing | NLinear | 적용 안 함 | 1.2648 | STL 보조 실험 안에서 큰 개선을 만든 구조 |
| STL 구조분해 보조 노트북: 최종 구조 | Exponential Smoothing | NLinear | LightGBM | 1.2275 | STL 보조 실험 안에서 가장 낮은 시험 오차 |
| 공통 반복 검증: 평균 시험 오차 최저 | RandomWalk_1step | 적용 안 함 | 적용 안 함 | 2.2191 | 동일 조건 반복 검증에서 가장 안정적이었던 benchmark |
...
3. 현재 자료에서는 단순 benchmark와 STL 계열이 더 안정적이었으므로, 논문 본문에서는 복잡한 조합의 최저 오차보다 반복 검증에서의 재현성을 더 앞세우는 편이 타당하다.
