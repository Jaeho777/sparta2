...
- 추가 점검용 반복 평가는 다섯 개 시작점(`2024-11-25`, `2025-02-17`, `2025-05-12`, `2025-08-04`, `2025-10-27`)에 대해 같은 `12주 검증 + 12주 시험` 반복 원점 규칙을 적용한 결과이다. 이 절의 평균 오차가 공식 단일 분할보다 크게 나온 이유는 더 어려운 구간과 더 짧은 학습 이력이 함께 포함되기 때문이다.
...
이 절의 평균 RMSE가 공식 단일 분할 결과보다 더 크게 나온 것은 자연스러운 결과이다. 이유는 세 가지이다. 첫째, 단일 분할은 `2025-10-27` 시작 한 구간만 평가하지만, 반복 평가는 다섯 개 시작점을 평균하므로 더 어려운 구간이 함께 포함된다. 둘째, 앞선 시작점일수록 학습에 사용할 수 있는 과거 길이가 더 짧다. 셋째, 반복 평가의 공통 비교 스크립트는 계산 가능성을 위해 transformer 학습 예산을 단일 분할 confirmatory 재학습보다 줄였다. 예를 들어 `2025-05-12` 시작 구간에서는 transformer 계열 시험 RMSE가 크게 상승했다. 따라서 이 절은 `모델이 갑자기 나빠졌다`는 뜻이 아니라, `한 번 좋게 나온 결과가 다른 시점에서도 반복되는가`를 점검하는 단계로 읽어야 한다.
...
추가 점검용 반복 평가는 단일 분할의 최고 성능을 재현하기 위한 절차가 아니라, 그 성능이 다른 시점에서도 유지되는지 확인하기 위한 절차이다. 따라서 평균 성능이 더 낮게 나오는 것은 이상한 현상이 아니다.

1. `평가 구간 수가 늘어났다`
   - 단일 분할은 `2025-10-27` 시작 한 번의 시험 구간만 본다.
   - 반복 평가는 다섯 개 시작점을 평균한다.
   - 따라서 특정 한 구간에 우연히 유리했던 조합은 반복 평가에서 평균 성능이 빠르게 낮아질 수 있다.

2. `시장 국면이 달랐다`
   - `2025-10-27` 시작 시험 구간은 비교적 완만한 구간이었고, 일부 조합이 이 구간에 특히 잘 맞았다.
   - 반면 반복 평가에는 더 변동성이 큰 시점과 구조 변화가 큰 시점이 함께 들어간다.
   - 이 차이는 특히 transformer 계열의 평균 시험 오차를 크게 늘렸다.

3. `학습 이력이 더 짧은 구간이 포함되었다`
   - 앞선 시작점으로 갈수록 train 구간이 짧아진다.
   - complex model은 학습 표본이 줄어들수록 분산이 커지기 쉽다.
   - 그 결과, 단일 분할에서는 좋아 보였던 조합이 반복 평가 평균에서는 불안정하게 나타날 수 있다.

4. `공통 반복 평가에서는 계산 예산을 줄였다`
