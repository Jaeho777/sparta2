#!/usr/bin/env python3
"""
Script to update the metrics table with clear model composition names
"""
import json

NOTEBOOK_PATH = "/Users/jaeholee/Desktop/sparta_2/sparta2_advanced.ipynb"

# New code for metrics calculation with model name mapping
...
def main():
    ...
    # Find the metrics calculation cell
    target_cell_index = None
    for i, cell in enumerate(notebook['cells']):
        if cell.get('cell_type') == 'code':
            source = ''.join(cell.get('source', []))
            if '# 성능 지표 계산' in source and 'all_preds.columns' in source and 'SPARTA2_RMSE' in source:
                target_cell_index = i
                break
    ...
    print(f"  - Updated metrics calculation cell at index {target_cell_index}")
    print("  - Added model name mapping for clear composition info")
    return True
